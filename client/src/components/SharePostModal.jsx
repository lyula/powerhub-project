...
export default function SharePostModal({ open, onClose, postUrl, onShare }) {
  const [copied, setCopied] = useState(false);
  // Extract postId from postUrl (assumes /post/:id)
  let postId = '';
  if (postUrl) {
    const match = postUrl.match(/\/post\/(\w+)/);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40 dark:bg-black dark:bg-opacity-70">
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl p-8 w-full max-w-md relative border border-gray-200 dark:border-gray-800">
        ...
        <h2 className="text-xl font-bold mb-4 text-center text-[#0bb6bc] dark:text-[#fc3a57]">Share Post</h2>
        <div className="mb-4">
        ...
        <div className="flex justify-between gap-2 mt-2">
          {SOCIALS.map(social => (
            <button
              key={social.name}
              onClick={() => handleSocialShare(social.url(postUrl))}
