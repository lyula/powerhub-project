...
const getIconBgColor = (type) => {
  switch (type) {
    ...
    case 'share':
      return 'bg-gradient-to-br from-purple-600 to-fuchsia-500';
    default:
...
const NotificationModal = ({ notifications, onClose, anchorRef, socket }) => {
  const [localNotifications, setLocalNotifications] = useState(notifications);
  const navigate = useNavigate();

  // ✅ Handle socket new notifications safely
  useEffect(() => {
    if (!socket) return;
  ...
  const handleNotificationClick = (notification) => {
    ...
    const { type, relatedContent } = notification;
    if (!relatedContent || !relatedContent.contentId) return;

    const { contentType, contentId } = relatedContent;
    ...
    switch (type) {
      ...
      case 'reply':
        if (contentType === 'video') {
          navigate(`/watch/${contentId}?scrollToComments=true`);
        } else if (contentType === 'post') {
          navigate(`/post/${contentId}?scrollToComments=true`);
        }
        break;
      case 'subscribe':
          ...
          <ul className="divide-y divide-gray-100/50 dark:divide-gray-800/50">
            {localNotifications.slice(0, 5).map((notif, idx) => (
              <li
                ...
                className={`p-4 flex items-start gap-3 cursor-pointer transition-all duration-200 hover:bg-gray-50 dark:hover:bg-gray-800/70 ${
                  !notif.read ? 'bg-blue-50/50 dark:bg-blue-900/20' : ''
                }`}
              >
                <div className="relative flex-shrink-0">
                  <img
                    src={notif.avatar || `https://picsum.photos/seed/${notif._id || idx}/40/40`}
